...
# Barra lateral persistente para regresar o alternar rápido
with st.sidebar:
    ...
    if st.button("📄 Generador de Cotizaciones", use_container_width=True):
...
if st.session_state.app_actual == "Menu":
    ...
    st.markdown("Bienvenido al centro de mando operativo. Por favor, selecciona la plataforma que deseas desplegar:")
    st.divider()
    ...
    with col1:
        ...
        st.caption("Cálculo automatizado de márgenes, diseño adaptivo de presupuestos en formato PDF y sincronización centralizada en Google Sheets.")
        if st.button("Desplegar Cotizador", key="launch_cot", use_container_width=True, type="primary"):
            cambiar_pantalla("Cotizaciones")
            
    with col2:
        st.markdown("### 📸 Evidencia Técnica")
        st.caption("Sistema de Evidencia Técnica Alducin Aire Acondicionado Industrial. Registro fotográfico de campo y captura de datos de mantenimiento.")
        if st.button("Desplegar Reportes", key="launch_rep", use_container_width=True, type="primary"):
            cambiar_pantalla("Reportes")
            
    with col3:
        st.markdown("### 🚀 Próxima Aplicación")
        st.caption("Módulo en reserva configurado y listo para recibir la integración de tu siguiente flujo de trabajo automatizado.")
...
# ==========================================
# VISTA 3: APP DE REPORTES FOTOGRÁFICOS
# ==========================================
elif st.session_state.app_actual == "Reportes":
    col_back, col_title = st.columns([1, 8])
    with col_back:
        if st.button("⬅️ Volver", key="back_to_menu_rep", use_container_width=True):
            cambiar_pantalla("Menu")
            
    st.title("Sistema de Evidencia Técnica Alducin Aire Acondicionado Industrial")
    st.subheader("Módulo de Captura de Mantenimiento y Reportes Fotográficos")
    st.divider()
    
    st.info("Este espacio está configurado y listo para recibir las funciones de carga de imágenes, generación automática de reportes de campo en PDF y guardado en base de datos.")
    
    # Estructura inicial del formulario técnico para que sea funcional
    with st.form("registro_evidencia_tecnica"):
        st.markdown("#### Datos de la Inspección")
        col_t1, col_t2 = st.columns(2)
        with col_t1:
            tecnico_nombre = st.text_input("Técnico Responsable")
            cliente_rep = st.text_input("Cliente / Empresa")
        with col_t2:
            fecha_inspeccion = st.date_input("Fecha del Servicio", date.today())
            orden_trabajo = st.text_input("Número de Orden de Trabajo (OT)")
            
        st.markdown("#### Registro Fotográfico")
        imagenes_cargadas = st.file_uploader("Selecciona la evidencia fotográfica del equipo", type=["jpg", "png", "jpeg"], accept_multiple_files=True)
        observaciones = st.text_area("Observaciones técnicas y diagnósticos del equipo")
        
        submit_reporte = st.form_submit_button("Generar Reporte de Evidencia")
        if submit_reporte:
            st.success("Formulario de pruebas estructurado correctamente. Listo para enlazar código de generación de PDF fotográfico.")

# ==========================================
# VISTA 4: PRÓXIMA APLICACIÓN (OTRA APP)
# ==========================================
elif st.session_state.app_actual == "OtraApp":
    col_back, col_title = st.columns([1, 8])
    with col_back:
        if st.button("⬅️ Volver", key="back_to_menu_otra", use_container_width=True):
            cambiar_pantalla("Menu")
            
    st.title("Módulo en Desarrollo")
    st.subheader("Espacio reservado para tu nueva herramienta automatizada")
    st.divider()
    
    st.warning("Esta sección está completamente enrutada. En cuanto decidas automatizar el siguiente proceso de tu flujo laboral, colocaremos el formulario y la lógica aquí dentro.")
